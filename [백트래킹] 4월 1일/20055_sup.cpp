...
int minusPosition(int n, int pos) { //인덱스 감소(벨트 회전)
    if (--pos >= 0) {
        return pos;
    }
    return n * 2 - 1; //위치가 0일 경우
}
...
void second(int n, int start, int end) {
    ...
    while (cur != start) { //end - 1 부터 start 까지 검사
        ...
        if (belt[cur].is_on && !belt[next].is_on && belt[next].power) { //현재 위치에 로봇이 있고, 다음 위치에 로봇이 없으며 내구성 1이상인 경우 -> 로봇 옮김
            belt[cur].is_on = false; //현 위치 로봇x
            belt[next].power--; //다음 위치 내구도 -1
            if (next != end) { //내리는 위치 아닐 경우만
                belt[next].is_on = true; //로봇 옮기기
            }
            if (belt[next].power == 0) { //내구도가 0이라면 종료 조건 ++
                zero_power++;
            }
        }
    }
...
void third(int start) {
    if (belt[start].power) { //올리는 위치의 내구도가 0이 아니라면 로봇 올림
        belt[start].is_on = true; //로봇 올리기
        belt[start].power--; //내구도 --
        if (belt[start].power == 0) { //내구도 0이라면 종료 조건 ++
            zero_power++;
        }
    }
}
...
int main() {
    ...
    //입력
    cin >> n >> k; //전체 칸 수, 종료 조건 칸 수 입력
    belt.assign(n * 2, { 0, false }); //벨트 내구도 벡터
    for (int i = 0; i < n * 2; i++) { //각 칸의 내구도 입력
        cin >> belt[i].power;
    }
    ...
    int step = 0;     //단계 수
    while (true) {
        start = minusPosition(n, start); //벨트 회전 -> 올리는 위치
        end = minusPosition(n, end); //벨트 회전 -> 내리는 위치
        if (belt[end].is_on) { //벨트 회전 후, 로봇이 내리는 위치에 있다면 내리기
            belt[end].is_on = false;
        }
        second(n, start, end); //로봇 이동
        third(start); //로봇 올리기
        step++; //단계++
        if (zero_power >= k) { //종료 조건(내구도 0인 칸 수)
            break;
        }
    }
