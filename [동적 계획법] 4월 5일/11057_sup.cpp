...
int sumLastCnt(vector<int>& arr) {
    int ans = 0; //합 구하기 위해 초기화
    for (int i = 0; i < arr.size(); i++) {
        ans += arr[i]; //합에 받은 배열 더하기
        ans %= MOD; //모듈러 연산
    }
    return ans;
}
...
int upNumberCnt(int n) {
    ...
    return sumLastCnt(dp[n]); //합을 더하기 위해 함수로 반환
}
...
int main() {
    ...
    //연산 & 출력
    cout << upNumberCnt(n); //upNumberCnt에서 연산 -> sumLastCnt에서 합해서 반환 -> upNumberCnt로 다시 반환해 출력
    return 0;
}