...
int main() {
    ...
    while (n--) {
        //입력
        cin >> guitar_string >> fret; //누를 음 입력

        //연산
        while (guitar[guitar_string].top() > fret) { //현재 스택 맨 위 프렛이 누를 프렛보다 위쪽인 경우
            ans++; //횟수 카운트
            guitar[guitar_string].pop(); //프렛에서 손가락 떼기
        }
        if (guitar[guitar_string].top() != fret) { //현재 스택 맨 위 프렛이 누를 프렛보다 아래일 경우
            ans++; //횟수 카운트
