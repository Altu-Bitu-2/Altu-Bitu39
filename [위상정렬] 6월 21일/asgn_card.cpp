...
ci ctrl(int row, int col, int dir, vector<vector<int>>& tmp) {
    while (true) {
        ...
        if (row < 0 || row >= SIZE || col < 0 || col >= SIZE) { //해당 방향에 카드가 하나도 없다면 그 방향의 가장 마지막 칸으로 이동
            return make_pair(row - dr[dir], col - dc[dir]); //해당 좌표 반환
        }
        if (tmp[row][col] != 0) { //누른 키 방향에 있는 가장 가까운 카드
            return make_pair(row, col); //해당 좌표 반환
        }
    }
}
...
int bfs(int r1, int c1, int r2, int c2, vector<vector<int>>& tmp) {
    vector<vector<int>> visited(SIZE, vector<int>(SIZE, 0)); //방문한 위치 파악
    queue<ci> q; //bfs 계산할 좌표 큐
    visited[r1][c1] = 1; //반드시 엔터를 누르게 될 것. 엔터를 미리 눌렀다 가정하고 1로 표시
    q.push({ r1, c1 }); //현 좌표 큐에 넣기

    while (!q.empty()) { //계산할 값이 없어질 때까지
        int row = q.front().first; //큐의 첫 x 좌표
        int col = q.front().second; //큐의 첫 y 좌표
        int dist = visited[row][col]; //이동 비용 초기화
        q.pop(); //좌표 계산할 예정이므로 삭제

        if (row == r2 && col == c2) { //목표 카드에 도달했다면
            return dist;
        }
        for (int i = 0; i < 4; i++) { //컨트롤 입력 이동
            pair<int, int> np = ctrl(row, col, i, tmp); //이동한 좌표
            if (visited[np.first][np.second]) { //아무것도 없다면
                continue; //다음 반복
            }
            visited[np.first][np.second] = dist + 1; //이동 비용 ++
            q.push(np); //해당 좌표 계산 위해 큐에 넣기
        }

        for (int i = 0; i < 4; i++) { //방향키 입력 이동
            int nr = row + dr[i], nc = col + dc[i]; //4방위 좌표 초기화
            if (nr < 0 || nr >= SIZE || nc < 0 || nc >= SIZE || visited[nr][nc]) { //보드 밖으로 이동하게 된다면
                continue; //다음 반복
            }
            visited[nr][nc] = dist + 1; //이동 비용++
            q.push({ nr, nc }); //해당 좌표 계산 위해 큐에 넣기
        }
    }
...
vector<vector<ci>> findCard(vector<vector<int>>& board) {
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            cnt = max(cnt, board[i][j]); //종류별 번호가 다름을 이용
    ...
    return card_pos; //카드 위치 반환
}

int solution(vector<vector<int>> board, int r, int c) {
    int answer = 1e9; //최솟값 도출을 위한 초기화
    ...
    for (int i = 0; i < card_cnt; i++) {
        seq[i] = i + 1; //카드는 1부터 시작
    }
    do { //가능한 모든 카드 순서에 대해
        for (int bit = 0; bit < (1 << card_cnt); bit++) { //0, 1번째 카드 중 어떤 카드를 먼저 뒤집을지 결정
            answer = min(answer, matchCard(bit, card_cnt, r, c, answer, seq, cards, board)); //최솟값을 계산해 이전 값과 비교 후 저장
        }
    } while (next_permutation(seq.begin(), seq.end())); //모든 경우의 수를 다할 때까지
    return answer; //최솟값 출력
}
 ...
 */
int main() {
    vector<vector<int>> board = { {1, 0, 0, 3}, //보드의 카드 배열
                                 {2, 0, 0, 0},
                                 {0, 0, 0, 2},
                                 {3, 0, 1, 0} };
    cout << solution(board, 1, 0); //키 조작 횟수 최솟값 계산해 출력
}