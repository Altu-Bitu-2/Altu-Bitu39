...
int main() {
    ...
    string s, e, q, t, name; //개총시작, 개총끝, 스트리밍끝, 학회원채팅시각, 학회원명 (':' 들어가므로 string)
    map<string, bool> m; //map<이름, 출석 여부>
    int ans = 0; //출석 확인된 학회원 인원 수

    //입력 & 연산
    cin >> s >> e >> q; //방송 관련 시각 입력
    ...
    //시각 입력 -> 시작 출석 시각 시 일단 true(출석)
    //         -> 종료 출석 시각 시 시작 출석 true(출석완료)이면 카운트
